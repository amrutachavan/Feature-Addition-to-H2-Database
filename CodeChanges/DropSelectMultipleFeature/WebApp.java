...
public class WebApp {
	...
	private String query() {
		...
		//code changes for alter table drop multiple column starts here
		// if the sql query string contains the keyword drop and column then execute it after our code changes
		if((sql.contains("drop")||sql.contains("DROP"))&&(sql.contains("column")||sql.contains("COLUMN"))){
			String inputThis="";
			//get the index of opening brackets containing the column names
			int i= sql.indexOf("(");			
			if(i!=-1){				
				char ch= sql.charAt(i);
				String colArr= new String();
				//read column names unless a closing bracket is encountered
				while(ch!=')'){			
					i++;
					ch=sql.charAt(i);
					while(ch!=','&&ch!=')'){							
					...
					//inputThis string is used to store the individual drop column queries
					if(inputThis==""){
						inputThis= "alter table student drop column "+colArr;
					}else{
						inputThis= inputThis+";"+"alter table student drop column "+colArr;
					}
					colArr="";
				}
				// if there are any other queries in the sql string after the drop query then add it to the inputTihs string
				inputThis=inputThis+sql.substring(i+1);				
				//assign the modified string to the intital sql string
				sql=inputThis+";";
			}			
		}
		
		//code changes for select * multiple table starts here
		// if the sql query string contains the keyword select and * then execute it after our code changes
		if((sql.contains("select")||sql.contains("SELECT"))&&(sql.contains("*"))&&(sql.contains("("))&&(sql.contains(""))){
			String inputThis="";
			//get the index of opening brackets containing the column names
			int i= sql.indexOf("(");			
			if(i!=-1){
				...
				while(ch!=')'){			
					...
					//inputThis string is used to store the individual drop column queries
					if(inputThis==""){
						inputThis= "select * from "+colArr;
					}else{
						inputThis= inputThis+";"+"select * from "+colArr;
					}
					colArr="";
				}
				// if there are any other queries in the sql string after the drop query then add it to the inputTihs string
