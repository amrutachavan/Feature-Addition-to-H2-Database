...
public class Parser {
	...
	public Prepared prepare(String sql) {
		...
		if ((sql.contains("DROP")||sql.contains("drop"))&&(sql.contains("COLUMN")||sql.contains("column"))){
			System.out.println("!!!!!!!!!!!!!!!!!!I got drop query");
			int i= sql.indexOf("(");
			int j=-1;
			if(i!=-1){
		...
		try {
			...
			if (hasMore) {
				...
				if (remaining.trim().length() != 0) {
					System.out.println("VVVVVVVVVVVVVVVVVV sql here :"+sql);
	...
	private String readColumnsIdentifier() {
		if (currentTokenType != IDENTIFIER) {
			throw DbException.getSyntaxError(sqlCommand, parseIndex,
					"identifier");
		}
	...
	private void initialize(String sql) {
		...
		if((sql.contains("Drop")||sql.contains("drop"))&&(sql.contains("column")||sql.contains("COLUMN"))){
        	...
        	for (int k=0;k<colArr.length;k++){
        		System.out.println("======>>>>>>>>>>"+colArr[k]);
        	}*/
			System.out.println("Got the delete column query");
	...
	private Prepared parseAlterTable() {
		...
		if (readIf("ADD")) {
			...
			// Changes made to the code as follows

			/*
			 * Here we first scan for the string between the '(' and ')'
			 * Then retrieve the string i.e column names 
			 * Pass the column names to the "dropSingleColumnConstraintsAndIndexes"
			 * to drop multiple columns.
			 * 
			 * Note: Here we store the columns in a String array and pass
			 * is to an arrach of columns
			 */
			/*int i=0;
			if (readIf("(")) {
				do {
					i++;
					columnName[count] = readColumnIdentifier();
					count++;
				...
				{
					boolean ifExists = readIfExists(false);
					System.out.println("Dropping column: "+columnName[k]);
					if (ifExists && !table.doesColumnExist(columnName[k])) {
						return new NoOperation(session);
					}

					command.setOldColumn(table.getColumn(columnName[k]));
					parsePrepared();
					k++;
					i--;
				}while(i>0);
