...
public class Parser {
 ...
 // H2 group 3 implemenation start
    private static final int ENUMLIST = 27;
    private static final int COLUMNENUMLIST = 26;
 // H2 group 3 implemenation end
 ...
 // H2 group 3 implemenation start
    private ArrayList<String> enumLists = new ArrayList<String>();
    private String enumIs = "";
    private String enumColumn = "";
    private String enumR = "";
 // H2 group 3 implemenation end
    ...
    public Command prepareCommand(String sql) {
        try {
        	 if(enumLists!= null && !enumLists.isEmpty()){
             	sql = enumR +")";
             }
        	Prepared p = parse(sql);
   ...
   // H2 group 3 implemenation - Start
    private Expression readExpressionEnum(ArrayList<String> enumList, String columnName){
    	Expression	r = readConditionEnum(enumList, columnName);
    	return r;
    	 
    }
    private Expression readConditionEnum(ArrayList<String> enumList, String columnName){
    	  currentTokenType = COLUMNENUMLIST;
          Expression r = readConcat();
          Expression r1 = r;
      	ArrayList<Expression> list = New.arrayList();
      	for(String s:enumLists){
      	r = ValueExpression.get(ValueString.get(s));
      	list.add(r);
      	}
          r = new ConditionIn(database, r1, list);
          enumR = r.toString();
          return r;
    }
 ...
 // H2 group 3 implemenation - End
    ...
    private Expression readTerm() {
        ...
        case VALUE:
         ...
         // H2 group 3 implemenation - start
        case COLUMNENUMLIST:
        	r = new ExpressionColumn(database, null, null, enumColumn);
        	//r = ValueExpression.get(ValueString.get(enumColumn));
        	break;
        	// H2 group 3 implemenation - end
        default:
            throw getSyntaxError();
 ...
 // H2 group 3 implementation start
    
    private void readEnumList(){
        if(enumLists.isEmpty()){
        	read("(");
        }
        else{
    ...
    private Column parseColumnWithType(String columnName) {
     ...
     // H2 group 3 implemenation  end
        ...
        if (dataType.supportsPrecision || dataType.supportsScale) {
         ...
         // H2 group 3 implemenation start
        } else if(dataType.jdbc.equals("Enum")){
        	enumLists = new ArrayList<String>();
        	readEnumList();
        }       	
             else if (readIf("(")) {
            readPositiveInt();
     ...
     // H2 group 3 implemenation start
        if(dataType.jdbc.equals("Enum")){
        	 column = new Column(columnName, type, enumLists, true);
        }
        else {
            ...
            if (!readIf(")")) {
                do {
                    ...
                    if (c != null) {
                     ...
                     // H2 group 3 implemenation start
                        if (readIf("CHECK") || (column.isEnum() && !column.getEnumList().isEmpty()) ) {
                            if(column.isEnum()){
                            	enumColumn = column.getName();
                            	Expression expr = readExpressionEnum(column.getEnumList(), column.getName());
